(() => {
  ...
  domReady(async () => {
    ...
    let shouldAutoDownload = false; // set true on user stop click
...
function triggerLocalDownload(blob, fileName) {
  try {
    // Most browsers require a user gesture; we only call this after the user clicks Stop.
...
function buildLocalFileName(blob) {
  ...
  return `${stamp}_${safeEns}_${safeAdj}.${ext}`;
    ...
    stopRecordingButton.addEventListener("click", async () => {
      armIdlePopup();
      // Flag ensures download is attempted only when Stop was clicked by a user gesture.
      ...
      return new Promise((resolve) => {
        ...
        mediaRecorder.onstop = () => {
          ...
          if (lastAudioBlob) {
            ...
            // Auto-download the file locally (best-effort).
            if (shouldAutoDownload) {
              const localName = buildLocalFileName(lastAudioBlob);
              triggerLocalDownload(lastAudioBlob, localName);
            }
