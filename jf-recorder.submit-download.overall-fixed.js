(() => {
  ...
  domReady(async () => {
    ...
    /************************************************************
     * OVERALL RATING (Average rating1–rating5 -> overallRating)
     * Notes:
     * - Averages only filled numeric values (blanks ignored)
     * - Uses event delegation so it works even if Airtable re-renders inputs
     ************************************************************/
    (function initOverallRatingAverage() {
      const RATING_IDS = new Set(["rating1","rating2","rating3","rating4","rating5"]);
      ...
      // Event delegation: survives dynamic DOM updates
      function delegatedHandler(e) {
        const t = e && e.target;
        if (!t || !t.id) return;
        if (RATING_IDS.has(t.id)) updateOverallRating();
      }

      document.addEventListener("input", delegatedHandler, true);
      document.addEventListener("change", delegatedHandler, true);

      // Initial + a few retries (Airtable sometimes hydrates fields after DOMContentLoaded)
      updateOverallRating();
      let tries = 0;
      const retry = setInterval(() => {
        ...
        const overall = safeGet("overallRating");
        const anyRating = safeGet("rating1") || safeGet("rating2") || safeGet("rating3") || safeGet("rating4") || safeGet("rating5");
        if ((overall && anyRating) || tries >= 20) clearInterval(retry);
      }, 250);
    })();
    /************************************************************
